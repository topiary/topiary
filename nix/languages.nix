## Topiary language configuration expressed as a Nix attribute set.
##
## This mirrors the content of `languages.ncl` so the source of truth can live
## in Nix. `generateNcl` (in `nix/utils/generate-ncl.nix`) converts this to a
## `.ncl` file suitable for use as Topiary's configuration via `-C` /
## `TOPIARY_CONFIG_FILE`.
