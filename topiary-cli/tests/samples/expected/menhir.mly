/* This file was sourced from the Menhir project test suite (pfff_python.mly).
   It is used as a test case for Menhir formatting in Topiary. */
...
| FROM name_and_level IMPORT "(" list_comma(import_as_name) ")" { [ImportFrom ($1, $2, $5)] }
...
expr_stmt:
| tuple(test_or_star_expr) { ExprStmt (tuple_expr $1) }
(* typing-ext: *)
| tuple(test_or_star_expr) ":" test { ExprStmt (TypedExpr (tuple_expr $1, $3)) }
...
funcdef: DEF NAME parameters return_type? ":" suite { FunctionDef ($1, $2, $3, $4, $6, []) }

async_funcdef: ASYNC DEF NAME parameters return_type? ":" suite { FunctionDef ($2, $3, $4, $5, $7, [] (* TODO $1 *)) }
...
classdef: CLASS NAME arglist_paren_opt ":" suite { ClassDef ($1, $2, $3, $5, []) }
...
decorator: "@" decorator_name arglist_paren2_opt NEWLINE { $1, $2, $3 }
...
for_stmt:
| FOR exprlist IN tuple(test) ":" suite { For ($1, tuple_expr_store $2, $3, tuple_expr $4, $6, []) }
| FOR exprlist IN tuple(test) ":" suite ELSE ":" suite { For ($1, tuple_expr_store $2, $3, tuple_expr $4, $6, $9) }

try_stmt:
| TRY ":" suite excepthandler+ { TryExcept ($1, $3, $4, []) }
| TRY ":" suite excepthandler+ ELSE ":" suite { TryExcept ($1, $3, $4, $7) }
| TRY ":" suite excepthandler+ ELSE ":" suite FINALLY ":" suite { TryFinally ($1, [TryExcept ($1, $3, $4, $7)], $8, $10) }
| TRY ":" suite excepthandler+ FINALLY ":" suite { TryFinally ($1, [TryExcept ($1, $3, $4, [])], $5, $7) }
| TRY ":" suite FINALLY ":" suite { TryFinally ($1, $3, $4, $6) }
...
| tuple(namedexpr_or_star_expr) { fun (t1, t2) -> CompList (t1, to_list $1, t2) }
...
sync_comp_for:
| FOR exprlist IN or_test { [CompFor (tuple_expr_store $2, $4)] }
| FOR exprlist IN or_test comp_iter { [CompFor (tuple_expr_store $2, $4)] @ $5 }
...
lambdadef_nocond: LAMBDA varargslist ":" test_nocond { Lambda ($1, $2, $3, $4) }
