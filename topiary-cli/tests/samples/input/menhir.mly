...
let messy_inline_rule ==
 X; { 5 }
 | y = Z ; { 6 }

// single-line classic rule with a stray leading pipe: the first production stays
// on the colon line, so the pipe must be dropped
single_line_stray: | AT { () } ;

// production group with an internal "|" (two producers sharing one semantic
// action); the internal bar is non-leading and must keep its surrounding spaces
grouped_producers: AT | AT { () } ;

// empty semantic action: the injected OCaml block is zero-width, which used to
// crash the injection pass ("Could not find leaf for injected ocaml span")
empty_action: AT { } ;

// inline modern alternatives stay compact; a stray leading pipe is dropped
let single_line_modern == | A; { 1 } | B; { 2 }

let single_line_modern_without_pipe == A; { 1 } | B; { 2 }

// partially split modern alternatives become fully vertical
let multi_line_modern == A; { 1 }
  | B; { 2 }
